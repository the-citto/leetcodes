...
# @benchmark.register
@common_params
def bm_lc_001_v1(state: benchmark.State) -> None:
    """Benchmark Two Sum."""
    n = state.range(0)
...
# @benchmark.register
@common_params
def bm_lc_001_v2(state: benchmark.State) -> None:
    """Benchmark Two Sum."""
    n = state.range(0)
    nums, target = setup_data(n)
    while state:
...
# @benchmark.register
@common_params
def bm_lc_001_v3(state: benchmark.State) -> None:
    """Benchmark Two Sum."""
    n = state.range(0)
    nums, target = setup_data(n)
    while state:
...
@benchmark.register
@common_params
def bm_lc_001_v4(state: benchmark.State) -> None:
    """Benchmark Two Sum."""
    n = state.range(0)
    nums, target = setup_data(n)
    while state:
